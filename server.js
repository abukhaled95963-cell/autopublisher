...
function filterSourceLinks(text) {
  if(!text) return '';
  return text
    // Replace with a space (not empty string) to prevent word merging
...
// ===== AI failure/refusal phrase detection =====
// Comprehensive list of AI failure/refusal phrases that must NEVER be published as posts.
// When the AI returns one of these (e.g. "no text provided", "I cannot translate"), the
// system falls back to the original content instead of publishing the failure message.
const AI_FAILURE_PHRASES = [
  // Arabic refusals
  'لا أستطيع','لا يمكنني','عذراً','آسف','أنصحك','مصادر موثوقة','لا أملك','غير قادر',
  // Arabic "no content" responses
  'لا يوجد نص','لا توجد نص','النص فارغ','النص المقدم فارغ','لم يتم تقديم نص','لم تقدم نصاً',
  'لا يوجد محتوى','لا توجد ترجمة','لا يوجد ما','لم يُقدم','لم تُقدم','لم يقدم لي','لم تقدم لي',
  'نص فارغ','محتوى فارغ','بدون نص','أرجو تقديم','يرجى تقديم','يرجى توفير','أرجو توفير',
  'الرجاء تقديم','الرجاء توفير','أرسل لي النص','أعطني النص','زودني بالنص','شارك النص',
  'لا يوجد نص للترجمة','لا يوجد نص لإعادة','لا يوجد نص للصياغة','لا يوجد نص لترجمته',
  'لا يمكن إعادة صياغة','لا يمكن ترجمة','تعذر إعادة','تعذرت ترجمة',
  // English refusals
  'I cannot','I am unable','I can\'t','I apologize','I\'m sorry','sorry, but',
  'no text provided','no content provided','please provide','kindly provide',
  'there is no text','there\'s no text','empty text','no text to','content is empty',
  // Generic error patterns
  'ERROR:','Error:','خطأ:','فشل:','ALL_AI_FAILED'
];

function containsFailurePhrase(text) {
  if(!text) return false;
  const lower = text.toLowerCase();
  return AI_FAILURE_PHRASES.some(p => lower.includes(p.toLowerCase()));
}

// ===== Telegram message text extractor (handles <br>, link previews, replies) =====
// Cheerio's .text() does NOT add separators around <br>, so "#عاجل<br>مديرية" becomes
// "#عاجلمديرية" with the words merged. This helper replaces <br> tags with newlines
// BEFORE calling .text() so blocks are properly separated. It also strips link previews,
// replies, and metadata so their duplicated text doesn't leak into the extracted message.
function extractTelegramText($, msgEl) {
  try {
    const msgClone = $(msgEl).clone();
    // Remove duplicated content sources first
    msgClone.find('.tgme_widget_message_link_preview, .link_preview_description, .link_preview_title, .link_preview_site_name, .tgme_widget_message_reply, .tgme_widget_message_metadata, .tgme_widget_message_footer').remove();
    const textEl = msgClone.find('.tgme_widget_message_text').first();
    if(!textEl.length) return '';
    // Replace <br>, <br/>, <br /> with newlines so adjacent inline blocks aren't glued together
    textEl.find('br').replaceWith('\n');
    // Block-level tags should also be separated
    textEl.find('p, div, li').each(function() {
      $(this).append('\n');
    });
    let text = textEl.text();
    // Normalize whitespace: collapse runs of horizontal spaces, but keep newlines
    text = text.replace(/[ \t]+/g, ' ').replace(/[ \t]*\n[ \t]*/g, '\n').replace(/\n{3,}/g, '\n\n').trim();
    return text;
  } catch(e) {
    console.error('extractTelegramText error:', e.message);
    return '';
  }
}

// ===== Duplicate-text detection within a single post =====
// Detects when the same content is repeated within one post (e.g. from Telegram link
// previews that duplicate the message text inside a card). Returns the deduplicated text.
function dedupeRepeatedText(text) {
  if(!text || text.length < 40) return text;
  let cleaned = text.trim();

  // Case 1: Exact full duplication - text repeated twice end-to-end with optional whitespace
  // (e.g. "abc abc" -> "abc"). Try splitting in half with various whitespace tolerances.
  const halfLen = Math.floor(cleaned.length / 2);
  for(let offset = -3; offset <= 3; offset++) {
    const splitAt = halfLen + offset;
    if(splitAt < 20 || splitAt >= cleaned.length - 20) continue;
    ...
    if(firstHalf.length >= 30 && firstHalf === secondHalf) {
      console.log('Dedup: removed exact full duplication (offset='+offset+')');
      return firstHalf;
    }
  }

  // Case 2: Substring-based duplication - take first 60 chars and check if they appear again later
  if(cleaned.length >= 80) {
    const probe = cleaned.substring(0, 60).trim();
    if(probe.length >= 30) {
      const secondOccurrence = cleaned.indexOf(probe, 30);
      if(secondOccurrence > 0) {
        // Found a duplicate of the opening - cut at that position
        const beforeDup = cleaned.substring(0, secondOccurrence).trim();
        if(beforeDup.length >= 30) {
          console.log('Dedup: found repeated probe at offset '+secondOccurrence);
          return beforeDup;
        }
      }
    }
  ...
  // Case 3: Multi-line duplication - check if consecutive lines/paragraphs are identical
  const lines = cleaned.split(/\n+/).map(l => l.trim()).filter(Boolean);
  if(lines.length >= 2) {
    const dedupedLines = [];
    let prev = '';
    for(const line of lines) {
      if(line !== prev) dedupedLines.push(line);
      prev = line;
    }
    if(dedupedLines.length < lines.length) {
      console.log('Dedup: removed '+(lines.length - dedupedLines.length)+' duplicate lines');
...
async function readTelegramChannel(channel) {
  ...
  try {
    ...
    $('.tgme_widget_message').each(function(i, msgEl) {
      if(i >= 10) return;
      // Use the helper that properly handles <br> tags and strips link previews
      let text = extractTelegramText($, msgEl);
      // Safety net: also dedupe at the text level in case any duplication slipped through
...
async function publishPost(post) {
  ...
  // Telegram - with safety gate
  if(tgToken && tgChat && post.rewritten_telegram) {
    const tgClean = (post.rewritten_telegram || '').replace(/<[^>]+>/g, '').trim();
    const tgWords = tgClean.split(/\s+/).filter(w => w.length > 1).length;
    if(containsFailurePhrase(tgClean) || tgClean.length < 15 || tgWords < 5) {
      logs.push({platform:'telegram', status:'error', message:'blocked: failure phrase or empty'});
      console.log('publishPost BLOCKED telegram post id:'+post.id+' - failure phrase or too short');
    } else {
      try {
  ...
  // Facebook direct - with safety gate
  if(fbToken && fbPageId && post.rewritten_facebook) {
    const fbClean = (post.rewritten_facebook || '').trim();
    const fbWords = fbClean.split(/\s+/).filter(w => w.length > 1).length;
    if(containsFailurePhrase(fbClean) || fbClean.length < 15 || fbWords < 5) {
      logs.push({platform:'facebook', status:'error', message:'blocked: failure phrase or empty'});
      console.log('publishPost BLOCKED facebook post id:'+post.id+' - failure phrase or too short');
    } else {
      try {
  ...
  // Make.com for Twitter/Instagram - with safety gate
  if(webhook) {
    const twContent = post.rewritten_twitter||post.rewritten_facebook||'';
    if(twContent && !containsFailurePhrase(twContent) && twContent.trim().length >= 10) {
      try {
      ...
      logs.push({platform:'twitter/instagram', status:'error', message:'blocked: failure phrase or empty'});
    }
  }
  ...
  try {
    ...
    for(const post of posts) {
      ...
      if(fbMode === 'asis') {
        ...
        // Clean up multiple spaces created by the replacements
        fbText = fbText.replace(/[ \t]{2,}/g, ' ');
        ...
        // Safety: even in as-is mode, check we're not publishing junk
        const asisWords = fbText.trim().split(/\s+/).filter(w => w.length > 1).length;
        if(asisWords < 5) { console.log('FB asis content too short, skipping for:', source.name); continue; }
      ...
      // Filter AI refusals / failure phrases (comprehensive check)
      if(containsFailurePhrase(fbText)) {
        console.log('FB: AI returned refusal/failure phrase, using cleaned original');
        fbText = text.replace(/https?:\/\/\S+/g,' ').replace(/t\.me\/\S+/g,' ').replace(/@[\w\d]+/g,' ').trim();
      }
      ...
      try {
        ...
        // FINAL SAFETY GATE: never publish AI failure / refusal messages
        if(containsFailurePhrase(fbText)) {
          console.log('FB BLOCKED publishing failure phrase from:', source.name, 'preview:', fbText.substring(0,80));
          db.prepare("INSERT INTO publish_log(post_id,platform,status,message) VALUES(?,'facebook','error','blocked: failure phrase')").run(pid);
          continue;
        }

        // Require minimum word count for a real post
        const fbWordCount = fbText.trim().split(/\s+/).filter(w => w.length > 1).length;
        if(fbWordCount < 5) {
          console.log('FB content too short ('+fbWordCount+' words), skipping for:', source.name);
          db.prepare("INSERT INTO publish_log(post_id,platform,status,message) VALUES(?,'facebook','error','blocked: too few words')").run(pid);
          continue;
        }
...
function cleanArabicOnly(text) {
  if(!text) return '';
  return text
    // Replace non-Arabic characters with a SPACE instead of empty string
    // This prevents words from merging together when Latin/Cyrillic/CJK text is removed
    .replace(/[^\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\d.,!?؟،؛:«»\-\(\)@#\n]/g, ' ')
    // Collapse multiple spaces created by the replacement above
    .replace(/[ \t]{2,}/g, ' ')
...
function fixArabicText(text) {
  ...
  t = t.replace(/،([^\s])/g, '، $1');
  t = t.replace(/\.([^\s\d])/g, '. $1');
  // FIX: Add space between Arabic and Latin/Cyrillic/CJK (and vice versa) to prevent word merging
  t = t.replace(/([\u0600-\u06FF])([A-Za-z\u0400-\u04FF\u4E00-\u9FFF])/g, '$1 $2');
  t = t.replace(/([A-Za-z\u0400-\u04FF\u4E00-\u9FFF])([\u0600-\u06FF])/g, '$1 $2');
  // Space between Arabic letters and digits (and vice versa)
  t = t.replace(/([\u0600-\u06FF])(\d)/g, '$1 $2');
  t = t.replace(/(\d)([\u0600-\u06FF])/g, '$1 $2');
  // FIX: Detect a hashtag immediately followed by another hashtag with no space ("#عاجل#مديرية")
  t = t.replace(/(#[\u0600-\u06FFA-Za-z0-9_]+)(#)/g, '$1 $2');
  // FIX: Common news-prefix hashtags that get glued to the next sentence when <br> is lost
  // (e.g. "#عاجلمديرية..." → "#عاجل\nمديرية..."). We use an explicit list of known prefixes
  // to avoid false positives on legitimate hashtags that contain Arabic words.
  const NEWS_PREFIX_HASHTAGS = ['عاجل','عاجلة','خبر','أخبار','اخبار','تنبيه','هام','مهم','بيان','حصري','تحديث','تطور','مباشر'];
  for(const tag of NEWS_PREFIX_HASHTAGS) {
    const pattern = new RegExp('(^|\\s)#'+tag+'([\\u0600-\\u06FF])', 'g');
    t = t.replace(pattern, '$1#'+tag+'\n$2');
  }
...
async function processArchiveMode(channel, publishTo, tgToken) {
  ...
  if(rules.mode === 'rewrite') {
    try {
      ...
      let aiResult = await callAI(prompt, 1500);
      // Validate the AI output before using it
      if(!aiResult || containsFailurePhrase(aiResult)) {
        console.log('Archive: AI returned failure/refusal, using original content');
        finalText = content;
      } else {
        finalText = fixArabicText(cleanRewrittenText(aiResult));
      }
  ...
  // FINAL SAFETY GATE for Archive Mode
  const archiveCheck = (finalText || '').replace(/<[^>]+>/g, '').trim();
  const archiveWords = archiveCheck.split(/\s+/).filter(w => w.length > 1).length;
  if(containsFailurePhrase(archiveCheck) || archiveCheck.length < 20 || archiveWords < 5) {
    console.log('Archive BLOCKED: failure phrase or insufficient content for @'+channel);
...
// Comprehensive list of AI failure/refusal phrases is defined near the top of the file (after isArabicText)
...
function validateRewrittenText(original, rewritten) {
  ...
  // Reject if contains any AI failure/refusal phrase
  if(containsFailurePhrase(trimmed)) {
    console.log('Validation failed: contains AI failure phrase');
    return null;
  }

  // Reject if it's just SKIP
  if(trimmed.toUpperCase() === 'SKIP') return null;

  // Reject if it's mostly non-Arabic when we expect Arabic
  // (Arabic letters should be > 30% of meaningful chars)
  const arabicChars = (trimmed.match(/[\u0600-\u06FF]/g) || []).length;
  const meaningfulChars = trimmed.replace(/[\s\d\W]/g, '').length;
  if(meaningfulChars > 0 && (arabicChars / meaningfulChars) < 0.3) {
    console.log('Validation failed: text is not Arabic enough');
    return null;
  }

  // Reject if word count is too small for a meaningful post
  const wordCount = trimmed.split(/\s+/).filter(w => w.length > 1).length;
  if(wordCount < 5) {
    console.log('Validation failed: too few words ('+wordCount+')');
    return null;
  }

  // Reject excessive repetition
  const sentences = trimmed.split(/[.؟!]/g).filter(s => s.trim().length > 15);
  ...
  // Reject if too short compared to original
  if(original && trimmed.length < original.length * 0.15 && original.length > 200) {
  ...
  try {
    ...
    try {
      ...
      $('.tgme_widget_message').each(function(i, el) {
        ...
        // Use the helper that properly handles <br> tags and strips link previews
        let text = extractTelegramText($, el);
        // Safety net: dedupe at the text level too
        text = dedupeRepeatedText(text);
    ...
    for(const post of posts.slice(0,3)) {
      ...
      if(mode === 'as-is') {
        ...
        try {
          if(!post.text || post.text.length < 10) {
            ...
            const prompt = isNonArabic
              ? `أنت محرر أخبار عربي محترف. ترجم هذا النص وأعد كتابته كخبر صحفي عربي احترافي.\n\nتعليمات:\n1. ترجم كل الكلمات للعربية - الروسية والصينية والإنجليزية وغيرها\n2. المصطلحات التقنية الشائعة كـ FPV تبقى مع شرح مختصر\n3. ابدأ بالمعلومة الأهم مباشرة بدون مقدمات\n4. استخدم لغة صحفية واضحة وسلسة\n5. لا تذكر المصدر أو الروابط\n6. إذا كان النص إعلاناً أو غير مفيد: أجب بكلمة SKIP فقط\n7. اكتب الخبر كاملاً ومتماسكاً\n\nالنص:\n${text.substring(0,1500)}\n\nاكتب الخبر المترجم والمحرر:`
              : `أنت محرر أخبار عربي محترف. حوّل هذا النص إلى خبر صحفي احترافي بالعربية الفصحى.\n\nأسلوب الكتابة: ${toneAr}\n\nتعليمات:\n1. ابدأ بالمعلومة الأهم مباشرة\n2. استخدم لغة صحفية واضحة وسلسة\n3. لا تترك أي كلمة غير عربية إلا المصطلحات التقنية الشائعة\n4. لا تذكر المصدر أو الروابط أو معرفات القنوات\n5. اكتب النص كاملاً ومتماسكاً بدون انقطاع\n6. لا تكرر الجمل\n\nالنص:\n${text.substring(0,1500)}\n\nاكتب الخبر المحرر الآن:`;
            let rewritten = '';
            try {
            ...
            if(!rewritten || rewritten.trim().length < 10) {
              ...
              if(isNonArabic) {
                ...
                // Use SPACE instead of empty string to prevent word merging
                rewritten = text
                  .replace(/https?:\/\/\S+/g,' ')
                  .replace(/t\.me\/\S+/g,' ')
                  .replace(/@[\w\d]+/g,' ')
            ...
            if(rewritten && rewritten.trim() === 'SKIP') {
              ...
              // Use comprehensive failure phrase check (includes refusals AND "no text" responses)
              if(!rewritten || containsFailurePhrase(rewritten)) {
      ...
      // Final dedupe pass right before publishing - separates any channel-link suffix first,
      // dedupes only the body, then re-attaches the suffix.
      try {
        const myChannelSuffix = (finalText && finalText.indexOf('\n\n📢 ') > 0) ? finalText.substring(finalText.indexOf('\n\n📢 ')) : '';
        const bodyOnly = myChannelSuffix ? finalText.substring(0, finalText.indexOf('\n\n📢 ')) : finalText;
        const dedupedBody = dedupeRepeatedText(bodyOnly);
        if(dedupedBody !== bodyOnly) {
          console.log('Final dedupe applied to @'+channel+' msg '+post.msgId+' (saved '+(bodyOnly.length - dedupedBody.length)+' chars)');
          finalText = dedupedBody + myChannelSuffix;
          // update DB with deduped version
          db.prepare("UPDATE posts SET rewritten_telegram=? WHERE id=?").run(finalText, pid);
        }
      } catch(e) { console.error('Final dedupe error:', e.message); }

      try {
        // FINAL SAFETY GATE: Never publish AI failure/refusal messages, empty, or garbage content
        const cleanCheck = (finalText || '').replace(/<[^>]+>/g, '').trim(); // strip HTML tags for the check
        const wordCheck = cleanCheck.split(/\s+/).filter(w => w.length > 1).length;
        if(containsFailurePhrase(cleanCheck) || (cleanCheck.length > 0 && cleanCheck.length < 15) || (cleanCheck && wordCheck < 3)) {
          console.log('BLOCKED publishing failure/garbage message to @'+channel+' msg '+post.msgId+': "'+cleanCheck.substring(0,80)+'"');
          db.prepare("INSERT INTO publish_log(post_id,platform,status,message) VALUES(?,?,?,?)").run(pid,'telegram','error','blocked: failure phrase or empty content');
          // If post had media, still try to forward the media without the bad caption
          if(post.hasMedia) {
            try {
              const mediaSent = await extractAndSendMedia(tgToken, tgChat, channel, post.msgId, '');
              if(mediaSent) {
                db.prepare("UPDATE posts SET status='published', published_at=datetime('now') WHERE id=?").run(pid);
                db.prepare("INSERT INTO publish_log(post_id,platform,status,message) VALUES(?,?,?,?)").run(pid,'telegram','success','media only (text blocked)');
              }
            } catch(e) {}
          }
          await new Promise(r=>setTimeout(r,800));
