...
async function processTGChannel(channel) {
  try {
    ...
    for(const post of posts.slice(0,3)) {
      ...
      // Send text (media handled by forwardMessage above for forward mode)
      if(finalText) {
        try {
          // If has media and mode is rewrite/as-is: forward original media then send rewritten text
          if(post.hasMedia && (mode==='rewrite'||mode==='as-is')) {
            // Forward just the media (will include original caption which we'll override with text)
            try {
              await axios.post(`https://api.telegram.org/bot${tgToken}/forwardMessage`,{
                chat_id: tgChat, from_chat_id: '@'+channel, message_id: post.msgId
              });
            } catch(e) {}
            // Then send rewritten text separately
            await axios.post(`https://api.telegram.org/bot${tgToken}/sendMessage`,{
          ...
          console.log('PUBLISHED from @'+channel+' ['+mode+'] to '+tgChat);
        } catch(e) {
