...
async function readTelegramChannel(channel) {
  ...
  try {
    ...
    $('.tgme_widget_message').each(function(i, msgEl) {
      if(i >= 10) return;
      // FIX: Clone & strip link previews / replies to avoid duplicated text from link cards
      ...
      let text = msgClone.find('.tgme_widget_message_text').first().text().trim();
  ...
  try {
    ...
    try {
      ...
      $('.tgme_widget_message').each(function(i, el) {
        ...
        // FIX: Clone the element and REMOVE link previews + replies before extracting text.
        // Otherwise the link preview's duplicated text gets concatenated, causing duplication.
        const msgClone = $(el).clone();
        msgClone.find('.tgme_widget_message_link_preview, .link_preview_description, .link_preview_title, .link_preview_site_name, .tgme_widget_message_reply, .tgme_widget_message_metadata, .tgme_widget_message_footer').remove();
        // Take only the FIRST text element to avoid joining multiple text blocks
        let text = msgClone.find('.tgme_widget_message_text').first().text().trim();
